...
app = FastAPI(
    ...
    **To obtain a token:** Use the `POST /api/auth/login` endpoint with credentials
    ...
    openapi_tags=[
        ...
        {
            "name": "Authentication",
            "description": "Autenticación y gestión de sesiones de administradores"
        },
        {
            "name": "Form Tokens",
            "description": "Validación de tokens de seguridad para acceso a formularios"
        },
        {
            "name": "Custom Forms",
            "description": "Gestión y consulta de formularios customizados"
        },
        {
            "name": "Deployment",
            "description": "Carga y deployment de paquetes de formularios (requiere autenticación)"
        }
    ]
)
...
async def validate_dashboard_token_endpoint(request: Request, data: ValidateDashboardTokenRequest):
    ...
    **Flow:**
    ...
    3. Backend decrypts the 's' parameter (TripleDES → TokenId)
